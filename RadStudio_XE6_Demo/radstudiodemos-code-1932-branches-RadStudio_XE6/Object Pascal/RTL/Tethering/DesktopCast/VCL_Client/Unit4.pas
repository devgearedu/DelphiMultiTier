...
type
  TForm4 = class(TForm)
    Image1: TImage;
    DesktopTheterManager: TTetheringManager;
    ...
    procedure DesktopTheterManagerRequestManagerPassword(const Sender: TObject;
      const RemoteIdentifier: string; var Password: string);
    procedure tmCastTimer(Sender: TObject);
    procedure DesktopTheterManagerEndAutoConnect(Sender: TObject);
...
procedure TForm4.DesktopTheterManagerEndAutoConnect(Sender: TObject);
begin
  CheckRemoteProfiles;
end;

procedure TForm4.DesktopTheterManagerRequestManagerPassword(
...
begin
  if DesktopTheterManager.RemoteProfiles.Count > 0 then
  begin
    for I := 0 to DesktopTheterManager.RemoteProfiles.Count - 1 do
    begin
      ConnectedProfiles := ConnectedProfiles + ' - ' + DesktopTheterManager.RemoteProfiles.Items[I].ProfileText;
    end;
