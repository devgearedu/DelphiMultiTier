...
function TTextDataSet.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
begin
  ...
  begin
    ...
    if Result = grOK then
    begin
      StrLCopy(PChar(Buffer), PChar(FData[FCurRec]), MaxStrLen);
...
begin
  ...
  case Field.DataType of
    ftString:
      StrLCopy(PChar(RecData), PChar(string(PAnsiChar(Buffer))), Field.Size);
    ftWideString:
      StrLCopy(PChar(RecData), PChar(Buffer), Field.Size);
    else
      Assert(false, 'Unsupported field type.');
  end;
...
begin
  ...
  if State = dsEdit then FData[FCurRec] := PChar(ActiveBuffer) else
  begin
    { If inserting (or appending), increment the bookmark counter and
      store the data }
    Inc(FLastBookmark);
    FData.InsertObject(FCurRec, PChar(ActiveBuffer), Pointer(FLastBookmark));
  end;
end;
...
procedure Register;
begin
  RegisterComponents('Samples', [TTextDataSet]);
end;
