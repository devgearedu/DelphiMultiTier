...
type
  ...
  private
    ...
    FGetDeptCommand: TDSRestCommand;
    FGetDeptCommand_Cache: TDSRestCommand;
  ...
  public
    ...
    function GetDept(const ARequestFilter: string = ''): TStream;
    function GetDept_Cache(const ARequestFilter: string = ''): IDSRestCachedStream;
...
const
  ...
  TServerMethods1_GetDept: array [0..0] of TDSRestParameterMetaData =
  (
    (Name: ''; Direction: 4; DBXType: 33; TypeName: 'TStream')
  );

  TServerMethods1_GetDept_Cache: array [0..0] of TDSRestParameterMetaData =
  (
    (Name: ''; Direction: 4; DBXType: 26; TypeName: 'String')
  );
...
function TServerMethods1Client.GetDept(const ARequestFilter: string): TStream;
begin
  if FGetDeptCommand = nil then
  begin
    FGetDeptCommand := FConnection.CreateCommand;
    FGetDeptCommand.RequestType := 'GET';
    FGetDeptCommand.Text := 'TServerMethods1.GetDept';
    FGetDeptCommand.Prepare(TServerMethods1_GetDept);
  end;
  FGetDeptCommand.Execute(ARequestFilter);
  Result := FGetDeptCommand.Parameters[0].Value.GetStream(FInstanceOwner);
end;

function TServerMethods1Client.GetDept_Cache(const ARequestFilter: string): IDSRestCachedStream;
begin
  if FGetDeptCommand_Cache = nil then
  begin
    FGetDeptCommand_Cache := FConnection.CreateCommand;
    FGetDeptCommand_Cache.RequestType := 'GET';
    FGetDeptCommand_Cache.Text := 'TServerMethods1.GetDept';
    FGetDeptCommand_Cache.Prepare(TServerMethods1_GetDept_Cache);
  end;
  FGetDeptCommand_Cache.ExecuteCache(ARequestFilter);
  Result := TDSRestCachedStream.Create(FGetDeptCommand_Cache.Parameters[0].Value.GetString);
end;
...
begin
  ...
  FGetDeptCommand.DisposeOf;
  FGetDeptCommand_Cache.DisposeOf;
