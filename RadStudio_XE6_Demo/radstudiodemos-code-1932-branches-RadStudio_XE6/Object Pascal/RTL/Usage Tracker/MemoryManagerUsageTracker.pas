...
//---------------------------------------------------------------------------
unit MemoryManagerUsageTracker;
...
type
  TfMemoryManagerUsageTracker = class(TForm)
...
function ShowMemoryManagerUsageTracker: TfMemoryManagerUsageTracker;
...
function ShowMemoryManagerUsageTracker: TfMemoryManagerUsageTracker;
begin
  Application.CreateForm(TfMemoryManagerUsageTracker, Result);
...
procedure TfMemoryManagerUsageTracker.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfMemoryManagerUsageTracker.RefreshSnapShot;
var
  LInd: integer;
...
procedure TfMemoryManagerUsageTracker.tTimerTimer(Sender: TObject);
begin
...
procedure TfMemoryManagerUsageTracker.FormCreate(Sender: TObject);
var
  LInd: integer;
  LFootNote: string;
begin
  ...
  sgBlockStatistics.Cells[3, 0] := 'Reserved Mem';
  sgBlockStatistics.Cells[4, 0] := 'Efficiency';
  for LInd := 0 to high(FMemoryManagerState.SmallBlockTypeStates) do
  begin
    if (LInd > 0) and (FMemoryManagerState.SmallBlockTypeStates[LInd - 1].UseableBlockSize = FMemoryManagerState.SmallBlockTypeStates[LInd].UseableBlockSize) then
      LFootNote := '*';
    sgBlockStatistics.Cells[0, LInd + 1] :=
      IntToStr(FMemoryManagerState.SmallBlockTypeStates[LInd].UseableBlockSize) + LFootNote;
  end;
...
procedure TfMemoryManagerUsageTracker.bCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfMemoryManagerUsageTracker.dgMemoryMapDrawCell(Sender: TObject; ACol,
...
procedure TfMemoryManagerUsageTracker.dgMemoryMapSelectCell(Sender: TObject; ACol,
...
begin
  ...
  case FMemoryMap[LChunkIndex] of
    csAllocated:
    begin
      eState.Text := 'MM Allocated';
    end;
    csReserved:
    begin
      eState.Text := 'MM Reserved';
    end;
    csSysAllocated:
    begin
